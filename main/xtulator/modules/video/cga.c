...
#define CGA_HOR_RES  640
#define CGA_VER_RES  400

uint8_t cga_font[4096];
uint32_t* cga_framebuffer;
...
int cga_init() {
	...
	cga_framebuffer = (uint32_t*) calloc(CGA_HOR_RES * CGA_VER_RES, sizeof(uint32_t));

	if (utility_loadFile(cga_font, 4096, "/sdcard/roms/video/cgachar.bin")) {
		debug_log(DEBUG_ERROR, "[CGA] Failed to load character generator ROM\r\n");
		return -1;
	}

	for (y = 0; y < CGA_VER_RES; y++) {
		for (x = 0; x < CGA_HOR_RES; x++) {
			cga_framebuffer[y * CGA_HOR_RES + x] = cga_color(CGA_BLACK);
		}
	}
		...
		for (scy = start_y; scy <= end_y; scy++) {
			...
			for (scx = start_x; scx <= end_x; scx++) {
				...
				if ((y == cursor_y) && (x == cursor_x) &&
					...
					cga_framebuffer[scy * CGA_HOR_RES + scx] = cga_color(attr & 0x0F);
				}
				else {
					...
					cga_framebuffer[scy * CGA_HOR_RES + scx] = cga_color(fontdata ? (attr & 0x0F) : (attr >> 4));
				}
			}
		}
		break;
	case CGA_MODE_TEXT_40X25:
		...
		for (scy = start_y; scy <= end_y; scy++) {
			...
			for (scx = start_x; scx <= end_x; scx += 2) {
				...
				cga_framebuffer[scy * CGA_HOR_RES + scx + 1] = cga_framebuffer[scy * CGA_HOR_RES + scx]; //double pixels horizontally
			}
		}
		break;
	case CGA_MODE_GRAPHICS_LO:
		for (scy = start_y; scy <= end_y; scy += 2) {
			...
			for (scx = start_x; scx <= end_x; scx += 2) {
				...
				cga_framebuffer[scy * CGA_HOR_RES + scx] = cga_color(cc);
				cga_framebuffer[scy * CGA_HOR_RES + scx + 1] = cga_framebuffer[scy * CGA_HOR_RES + scx];
				cga_framebuffer[(scy + 1) * CGA_HOR_RES + scx + 1] = cga_framebuffer[scy * CGA_HOR_RES + scx];
				cga_framebuffer[(scy + 1) * CGA_HOR_RES + scx] = cga_framebuffer[scy * CGA_HOR_RES + scx];
			}
		}
		break;
