...
#define VGA_HOR_RES 1024
#define VGA_VER_RES 1024

VGADAC_t vga_DAC;
uint32_t* vga_framebuffer = NULL;
...
int vga_init() {
	...
	vga_framebuffer = (uint32_t*) calloc(VGA_HOR_RES * VGA_VER_RES, sizeof(uint32_t));
	...
	for (y = 0; y < 400; y++) {
		for (x = 0; x < 640; x++) {
			vga_framebuffer[y * VGA_HOR_RES + x] = vga_color(0);
		}
	}
	...
	TaskHandle_t handle;
    //xTaskCreate(&vga_render_task, "Audio player", 4096, NULL, 10, &handle);
	xTaskCreatePinnedToCore(&vga_render_task, "VGA", 4096, NULL, 10, &handle, 1);
	...
	VBIOS = (uint8_t*) malloc(32768);

	if (utility_loadFile(VBIOS, 32768, "/sdcard/roms/video/et4000.bin")) {
		return -1;
	}
		...
		for (scy = start_y; scy <= end_y; scy++) {
			...
			for (scx = start_x; scx <= end_x; scx++) {
				...
				if ((y == cursor_y) && (x == cursor_x) &&
					...
					vga_framebuffer[scy * VGA_HOR_RES + scx] = color32;
				}
				else {
			...
			for (scx = start_x; scx <= end_x; scx += xscanpixels) {
				...
				for (yadd = 0; yadd < yscanpixels; yadd++) {
					for (xadd = 0; xadd < xscanpixels; xadd++) {
						vga_framebuffer[(scy + yadd) * VGA_HOR_RES + (scx + xadd)] = color32;
					}
				}
			}
		}
